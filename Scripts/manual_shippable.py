...
from Shippable_complete import Shippable_complete

def Shippable():

    delete_local_files()
    file = path()+'\Files\\Shippable_'+format_date(3)+'.xlsx'
    ...
    #ship_status = pd.read_excel(path()+'\Files\\SHIP_STATUS.xlsx',sheet_name='SHIP STATUS')
    ...
    #ship_pivot = pd.pivot_table(master_summary_sc,index = ['COMPLEXITY CATEGORY'],values = ['OPEN QTY'],aggfunc = 'sum',margins_name = 'SHIPPABLE')
    ...
    #send_email('ecmms.OM@FII-NA.com ; ecmms.shipping@fii-na.com','','Shippable '+format_date(4),ship_pivot)
    send_email('rodrigo.bedolla@fii-na.com','','Shippable '+format_date(4),ship_pivot)
