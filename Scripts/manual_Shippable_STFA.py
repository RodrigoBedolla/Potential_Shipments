...
def Shippable_complete():
    ...
    while datetime.datetime.fromtimestamp(os.path.getmtime(share_path()+'\OM_RPAs_Files\Backup\SHIP_STATUS\SHIP_STATUS.xlsx')).date() < datetime.datetime.now().date():
        ...
        time.sleep(20)
    '''
    previous_master_share() #OUTLOOK CONFIGURATION 09192023
    shutil.copy(share_path()+'\Master Template\Master.xlsx',path()+'\Files\Previous_Master.xlsx')
    ...
    #--------OPEN QTY VALIDATION------------
    #master_summary = Ship_History(master_summary)
    ...
    #--------OPEN QTY VALIDATION------------
    #master_shippped = Ship_History(master_shippped)
    ...
    #send_email('erik.carbajalr@FII-NA.com','','Shippable '+format_date(4),ship_pivot)
