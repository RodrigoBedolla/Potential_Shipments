...
def potential_shipments():
    ...
    mail_format = pd.read_excel(path()+r'\Files\Shippable_'+str(format_date(3))+'.xlsx', sheet_name='SHIPPABLE',usecols=['WORK ORDER','CLOSED DATE','AGING','CATEGORY','PRIORITY','ERROR PRICE'])
    master_summary = pd.read_excel(path()+r'\Files\Shippable_'+str(format_date(3))+'.xlsx', sheet_name='RAWDATA')
    master_base = pd.read_excel(share_path()+r'\Master Template\master_base.xlsx', usecols=['WORK ORDER','STANDALONE']).drop_duplicates(subset='WORK ORDER')
    previous_master = pd.read_excel(path()+r'\Files\Previous_Master.xlsx', usecols=['WORK ORDER','STATUS']).drop_duplicates(subset='WORK ORDER')
    df5 = pd.read_excel(path()+r'\Files\Shippable_'+str(format_date(3))+'(CA).xlsx')
    ship_status = pd.read_excel(share_path()+r'\OM_RPAs_Files\Backup\SHIP_STATUS\\SHIP_STATUS.xlsx',sheet_name = 'SHIP STATUS',usecols=['WORK ORDER','OPEN FAILURE','HARDWARE FAILURES'])
    ...
    master_summary = master_summary.merge(mail_format.drop_duplicates(subset=['WORK ORDER']),on='WORK ORDER', how='left').reset_index(drop=True)
    master_summary.fillna('-', inplace=True)
    master_summary = master_summary.drop_duplicates(subset='WORK ORDER').reset_index(drop=True)
    master_summary.to_excel(path()+r'\Files\Qlik_Shippable_'+str(format_date(3))+'.xlsx', index=False)

    shutil.copy(path()+r'\Files\Qlik_Shippable_'+str(format_date(3))+'.xlsx',share_path()+r'\Qlik_Files')

    shutil.copy(path()+r'\Files\Qlik_Shippable_'+str(format_date(3))+'.xlsx','\\\\10.19.16.56\Order Management\OM Projects\Qlik_Files')
#potential_shipments()